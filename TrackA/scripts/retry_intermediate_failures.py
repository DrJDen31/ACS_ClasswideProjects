#!/usr/bin/env python3
"""
Retry failed intermediate size configurations with extended timeout
Similar to retry_failed_benchmarks.py but for 50K/500K sizes
"""
...
OUTPUT_FILE = SCRIPT_DIR / '../results/raw/intermediate_retry_results.csv'

# Very high timeout for slow coarse-grained 500K configs
TIMEOUT_SECONDS = 600  # 10 minutes

def load_failed_configs():
    """Load configurations that failed in the intermediate run"""
    failed = []
...
def main():
    print("="*70)
    print("Project A4 - Retry Failed Intermediate Size Configs")
    print("="*70)
    ...
    if not INTERMEDIATE_RESULTS.exists():
        print(f"ERROR: Intermediate results not found at {INTERMEDIATE_RESULTS}")
        print("Run run_intermediate_sizes.py first!")
        sys.exit(1)
    ...
    if len(failed_configs) == 0:
        print("No failed configurations found! All succeeded.")
    ...
    for config in failed_configs:
        print(f"  {config['strategy']:6s} {config['workload']:6s} "
              f"t={config['threads']:2d} n={config['size']:6d} rep={config['rep']}")
    print()
    
    # Estimate time
    avg_time_per_config = 180  # Conservative: 3 minutes
    ...
    with open(OUTPUT_FILE, 'w', newline='') as f:
        ...
        for config in failed_configs:
            ...
            print(f"[{run_count}/{len(failed_configs)}] {config['strategy']:6s} {config['workload']:6s} "
                  f"t={config['threads']:2d} n={config['size']:6d} rep={config['rep']} ({eta_str})", end=' ... ')
            sys.stdout.flush()
    ...
    print("="*70)
    print()
    print("Next step: Merge all results with final merge script")
    print("  python3 final_merge_all.py")
